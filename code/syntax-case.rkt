...
(define-syntax (my-class stx)
  (syntax-case stx ()
    [(my-class superclass . rest)
     (let* ([super (syntax->datum #'superclass)]
            [supers (cond [(symbol? super) (list super)]
                         [(null? super) (list object%)]
                         [else super])]
            [args (syntax->datum #'rest)])
       (expand! (symbol? super) supers args stx))]))
       ; (datum->syntax stx (append (list 'class super) args)))]))
...
(define (expand! do-not-touch? supers args stx)
  ; create meta object
  (let ([meta (make-metaobject supers args)])
    (update-generic-functions meta)
    ; create actual class object
    (let ([obj (make-classobject do-not-touch? supers args meta stx)])
...
(define (make-classobject do-not-touch? supers args meta stx)
  ...
  (let ([args (filter (negate defgeneric?) args)])
    (if do-not-touch?
        ...
        (datum->syntax stx (append '(class) supers args))
        ; else, put together the superclass
        (let ([superclass (datum->syntax stx (append '(class object% (super-new))
